...
export async function run(): Promise<void> {
  const keychainName: string = core.getInput('keychain-name', { required: true }) + '.keychain'
  const keychainPassword: string = core.getInput('keychain-password', { required: true })
  const keychainTimeout: number = parseInt(core.getInput('keychain-timeout', { required: true }))
    ...
    await core.group(
      ...
      async () => {
        for (let i = 0; i < signingCertificates.length; i++) {
          ...
          await importCertificate(certificatePath, signingCertificatePassphrase, keychainName, keychainPassword)
        }
        ...
        if (core.isDebug()) {
          ...
          core.info(`\x1b[33mCertificates in ${keychainName}: \n\x1b[0m` + certificates.map(cert => `\x1b[33m  * ${cert}\x1b[0m`).join('\n'))
        }
      }
    )
  } catch (error) {
